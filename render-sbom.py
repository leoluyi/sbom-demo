...
_SDT_CSS_URL = "https://cdn.jsdelivr.net/npm/simple-datatables@9/dist/style.min.css"
_SDT_JS_URL = "https://cdn.jsdelivr.net/npm/simple-datatables@9"
_SDT_INIT = """\
<script>
document.addEventListener('DOMContentLoaded', function() {
  document.querySelectorAll('[data-datatable]').forEach(function(el) {
    var rows = el.tBodies[0] ? el.tBodies[0].rows.length : 0;
    new simpleDatatables.DataTable(el, {
      searchable: rows > 15,
      paging: rows > 15,
      perPage: 25,
      perPageSelect: rows > 15 ? [10, 25, 50, 100] : false
    });
  });
});
</script>"""
...
.datatable-wrapper .datatable-bottom { padding:8px 0; font-size:13px; color:var(--muted); }
.datatable-wrapper .datatable-search input { padding:6px 10px; border:1px solid var(--line);
  border-radius:6px; font-size:13.5px; font-family:inherit; outline:none; }
.datatable-wrapper .datatable-search input:focus { border-color:var(--accent); }
.datatable-wrapper .datatable-selector { padding:4px 8px; border:1px solid var(--line);
  border-radius:6px; font-size:13px; font-family:inherit; }
.datatable-wrapper .datatable-info { font-size:13px; }
""".strip()
...
def _doc(title: str, body: str) -> str:
    return (
        ...
        f'<link rel="stylesheet" href="{_SDT_CSS_URL}">\n'
        "</head><body>\n"
        f'<div class="wrap">\n{body}\n</div>\n'
        f'<script src="{_SDT_JS_URL}"></script>\n'
        f"{_SDT_INIT}\n"
        "</body></html>\n"
    )
...
def _components_section(view: ArtifactView) -> str:
    ...
    return f'<h2>Components 清單 ({view.total})</h2><table data-datatable><thead>{head}</thead><tbody>{body}</tbody></table>'
...
def _licenses_section(view: ArtifactView) -> str:
    ...
    return f'<h2>Licenses 清單 ({view.distinct_licenses})</h2><table data-datatable><thead>{head}</thead><tbody>{body}</tbody></table>'
...
def render_index(views: list[ArtifactView]) -> str:
    ...
    body_html = (
        ...
        f'<h2>Artifacts</h2><table data-datatable><thead>{head}</thead><tbody>{body}</tbody></table>'
